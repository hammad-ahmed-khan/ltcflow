...
module.exports = async (req, res) => {
  try {
    ...
    const unreadMissedCallsCount = await MissedCall.countDocuments({
      calleeId: userId,
      companyId: companyId,
      cleared: false,
      retrieved: false, // ✅ Use existing 'retrieved' field like read state
      timestamp: { $gt: UNREAD_CUTOFF_DATE }, // ✅ Same cutoff logic
    });

    console.log(`📞 Unread Missed Calls: ${unreadMissedCallsCount}`);
    ...
    res.json({
      ...
      unreadRooms,
      unreadGroups,
      unreadMissedCalls: unreadMissedCallsCount, // ✅ ADD THIS
      totalUnread:
        unreadRooms.length + unreadGroups.length + unreadMissedCallsCount,
    });
  } catch (error) {
