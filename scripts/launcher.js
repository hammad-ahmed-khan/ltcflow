...
const backendOutput = {
  ...
  MONGO_DATABASE: "ltcflow",
  MAILER_ENABLED: false,
};

let frontendOutput = {
  VITE_SITE_TITLE: "LTC Flow",
  VITE_BACKEND_URL: "http://localhost",
  VITE_DEMO: false,
  VITE_SITE_BRAND: "LTC Flow",
...
(async () => {
  ...
  console.log("");
  console.log("LTC Flow".yellow);
  console.log(`LTC Flow v${pkg.version} Installer`.yellow);
  console.log("");
  ...
  if (arg === "setup") {
    console.log(
      "LTC Flow needs to know the public ip address of your machine.".cyan
    );
    console.log(
      "This is required for the meeting system to work, as traffic will be routed via UDP or TCP through such ip address."
    );
    console.log(
      'If you don\'t know your ip address, run "ping elderberry.example.com" (replace with your domain) from a local terminal.'
    );
    ...
    console.log("");
    console.log(
      "Are you using a mapped to public ip address, instead of a public one?"
        .cyan
    );
    console.log(
      "Should be YES for AWS, Azure, Google Cloud and other public cloud instances."
    );
    console.log(
      "Should be NO for OVH, DigitalOcean, Hetzner and other VPS providers."
    );
    ...
    console.log("");
    console.log(
      "LTC Flow requires a domain, such as elderberry.example.com.".cyan
    );
    console.log(
      "Make sure your DNS configuration gets properly propagated before moving forward."
    );
    ...
    console.log(
      "You will need an admin user account in order to manage other users."
    );
    console.log(
      "You can now enter username, email, password, first name and last name for such account."
    );
    console.log(
      "If you leave them blank, defaults in square parenthesis [] will be used."
    );
    ...
    console.log(
      "Your email will be used to generate a Let's Encrypt SSL certificate for the domain."
    );
    ...
    console.log(
      "You can input your own (random) set of characters or you can leave blank to generate a random secret."
    );
    ...
    console.log("");
    console.log(
      "Do you want to install nginx reverse proxy and SSL with Let's Encrypt?"
        .cyan
    );
    console.log("YES if you are running this on a clean server (recommended).");
    console.log(
      "NO if you are running other apps on this machines. You will have to complete the installation manually."
    );
    ...
    if (nginx) {
      ...
      console.log(
        "It will also be the email related to the admin user account."
      );
  ...
  if (["setup", "rebuild"].includes(arg)) {
    ...
    if (!os.includes("Ubuntu")) {
      ...
      console.log(
        "If you are running this on Ubuntu, please contact LTC Flow Support."
      );
      return process.exit(0);
    }

    if (
      !version.includes("22.04") &&
      !version.includes("20.04") &&
      !version.includes("18.04")
    ) {
      console.log("This is not a supported Ubuntu version.".red);
      console.log(
        "The only supported versions are 22.04 LTS, 20.04 LTS and 18.04 LTS"
      );
      console.log(
        "If you are running this on a correct version, please contact LTC Flow Support."
      );
      return process.exit(0);
    }
    ...
    console.log(
      `${arg === "setup" ? "installing" : "rebuilding"} LTC Flow backend...`
        .yellow
    );
    console.log("installing backend node modules...");
    console.log(
      "this might take a while, depending on your machine cpu, ram and connection speed"
    );
    console.log(
      "(might be even 10-15 minutes, please keep calm and wait patiently)"
    );
    exec("cd ../backend && yarn --prod --frozen-lockfile");
    console.log("starting backend...");

    try {
      exec("pm2 delete --silent LTCFlow", { stdio: "pipe" });
    } catch (e) {}
    exec("cd ../backend && pm2 start index.js --name LTCFlow");
    exec("pm2 save");
    exec("pm2 startup");
    console.log("LTC Flow backend started".green);

    console.log("");

    console.log(
      `${arg === "setup" ? "installing" : "rebuilding"} LTC Flow frontend...`
        .yellow
    );
    ...
    console.log("LTC Flow frontend ok".green);
  }
  ...
  if (arg === "restart") {
    try {
      exec("pm2 restart LTCFlow", { stdio: "pipe" });
    } catch (e) {}
  }

  if (arg === "stop") {
    try {
      exec("pm2 stop LTCFlow", { stdio: "pipe" });
    } catch (e) {}
  }
  ...
  if (arg === "setup" && nginx) {
    ...
    try {
      exec(
        "sudo ln -s /etc/nginx/sites-available/elderberry.conf /etc/nginx/sites-enabled/elderberry.conf"
      );
    } catch (e) {}
    try {
  ...
  if (arg === "stop") {
    console.log("");
    console.log(`LTC Flow has been stopped.`.green);
    console.log("");
  } else {
    console.log("");
    console.log(
      `LTC Flow v${pkg.version} ${
        arg === "setup" ? "setup" : "restart"
      } complete!`.green
    );
    if (arg === "setup") {
      console.log(`You should now be able to access LTC Flow at ${domain}`);
    }
    console.log("");
  }
})();
